param([Parameter(Mandatory=$True)] [string]$configPath,
    ...
    ForEach($add in $xml.configuration.appSettings.add)
    {
		...
		if($add.key -eq "InstrumentationKey")
	    {
		    Write-Output "Found matching variable for key: $($add.key)"
		    Write-Output "Replacing value $($add.value)  with $instrumentationKey"
            
		    $add.value = $instrumentationKey
	    }
		elseif($add.key -eq "AuthenticationMode")
	    {
		    Write-Output "Found matching variable for key: $($add.key)"
